...
result === null || result === void 0 ? void 0 : result.addEventListener("click", function () {
    ...
    if (isNaN(Number(expr))) {
        ...
        switch (true) {
            ...
            case regExp.root.test(expr):
                newExpr = expr.replace(regExp.root, (match, a, b) => {
                    return Math.pow(Number(b), 1 / Number(a)).toString();
                });
                return evaluate(newExpr);
            case regExp.ylogx.test(expr):
                newExpr = expr.replace(regExp.ylogx, (match, a, b) => {
